...
CREATE TABLE ParkingSlots (
    ...
    slot_size VARCHAR(7) CHECK (slot_size IN ('Small', 'Medium', 'Large')),
    slot_type VARCHAR(9) CHECK (slot_type IN ('Covered', 'Uncovered', 'Indoor', 'Outdoor')),
    slot_availability VARCHAR(9) CHECK (slot_availability IN ('Available', 'Occupied')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE Bookings (
    ...
    booking_status VARCHAR(10) CHECK (booking_status IN ('Booked', 'Cancelled', 'Expired', 'Completed')),
    payment_amount DECIMAL(10, 2),
    payment_status VARCHAR(9) CHECK (payment_status IN ('Pending', 'Approved', 'Declined')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES Users(user_id),
...
CREATE TABLE Payments (
    ...
    payment_method VARCHAR(11) CHECK (payment_method IN ('Credit Card', 'Debit Card', 'Paypal')),
    payment_amount DECIMAL(10, 2),
    payment_status VARCHAR(9) CHECK (payment_status IN ('Pending', 'Approved', 'Declined')),
...
CREATE TABLE Ratings (
    ...
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES Users(user_id),
    FOREIGN KEY (center_id) REFERENCES ParkingCenters
)
